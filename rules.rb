...
def testCanary
	...
	rulesSet << Rule.new( "border1", nil, [Split.new(YAxis,[[RelativeSize.new(1),"border2"],[AbsoluteSize.new(0.7),"delete"]])] )
	...
	rulesSet << Rule.new( "wallc", nil, [Split.new(YAxis,[[AbsoluteSize.new(1),"groundFloor"],Snap.new("level",0.5),[RelativeSize.new(1),"levels"]])] )
	...
	rulesSet << Rule.new( "levels", nil, [Tile.new(YAxis,1,"level",Snap.new("level",0.5),"level")] )
	rulesSet << Rule.new( "level", Proc.new{|node| node.shape.scope.x >= 1 and node.shape.scope.y >= 0.8}, [Tile.new(XAxis,0.8,"window",nil,"entrance")] )
